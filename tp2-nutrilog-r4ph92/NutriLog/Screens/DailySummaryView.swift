        ...
        NavigationSplitView {
            List {
                ...
                ForEach(MealType.allCases, id: \.self) { meal in
                    Section {
                        MealView(entries: entries(for: meal))   // ⬅️ on passe les items
                    } header: {
            ...
            .sheet(isPresented: $showAddForm) {
                AddMealView() // laisse tel quel si ta vue d'ajout existe. Sinon, mets un placeholder.
            }
        } detail: {
...
extension DailySummaryView {
    ...
    private func entries(for meal: MealType) -> [FoodEntry] {
        // si tu veux *vraiment* juste aujourd’hui, ajoute un filtre de date ici plus tard
